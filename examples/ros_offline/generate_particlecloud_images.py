...
def plot_particles(
    list_of_particles, output_path, image_width=800, image_height=600
):
    """
...
output_dir = "particle_cloud_images"
os.makedirs(output_dir, exist_ok=True)

# Iterate over all messages and save images
for message_number in range(3):# range(len(particle_cloud)):
    ...
    output_path = os.path.join(
        output_dir, f"particles_plot_{message_number}.png"
    )
    plot_particles(
        list_of_particles, output_path, image_width=800, image_height=600
    )
